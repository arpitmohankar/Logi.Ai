...
const Map = ({
  ...
  const [mapTypeCtrlOptions, setMapTypeCtrlOptions] = useState({});
  const [map, setMap] = useState(null);
  const [userLocation, setUserLocation] = useState(null);
  const [directions, setDirections] = useState(null);
  const { mapType, setMapCenter, setMapZoom , setMapType } = useMapStore();

  // Use the useJsApiLoader hook instead of LoadScript
  const { isLoaded, loadError } = useJsApiLoader({
    googleMapsApiKey: process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY,
    libraries,
    region: 'IN',      // bias map results to India
    language: 'en'     // UI labels in English
  });
...
useEffect(() => {
  ...
  const onLoad = useCallback((mapInstance) => {
     ...
     // Now that google.maps is defined, build & stash our typeControlOptions
      if (window.google && window.google.maps) {
        setMapTypeCtrlOptions({
     ...
     mapInstance.setOptions({
       restriction: {
         latLngBounds: { north: 37.6, south: 6.5, west: 68.0, east: 97.3 },
         strictBounds: false
       }
   });
   }, [setMapType]);
  ...
  return (
    <GoogleMap
      ...
      center={center || (userLocation ? userLocation : { lat: 21.1458, lng: 79.0882 })}
      zoom={zoom}
      onLoad={onLoad}
